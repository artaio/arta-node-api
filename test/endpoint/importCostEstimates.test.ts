...
import {
  type ImportCostEstimateCreateBodyDirect,
  type ImportCostEstimateCreateBodyFromQuote,
  ImportCostEstimatesEndpoint,
} from '../../lib/endpoint/importCostEstimates';
import * as helper from './helper';
import type { ImportCostEstimate } from '../../lib';

describe('tests Import Cost Estimates Arta endpoint', () => {
  const responseMock = {
  ...
  it('should have get, create and list', async () => {
    ...
    } satisfies ImportCostEstimateCreateBodyDirect;
  ...
  it('should create from quote_id', async () => {
    const requestConfig = { path, clientMock, endpoint };
    const createPayload = {
      quote_id: '8395a470-58df-4c2d-b5b5-7b76e5acdb61',
      currency: 'USD',
    } satisfies ImportCostEstimateCreateBodyFromQuote;
    await helper.testCreate(
      createPayload,
      'import_cost_estimate',
      requestConfig,
